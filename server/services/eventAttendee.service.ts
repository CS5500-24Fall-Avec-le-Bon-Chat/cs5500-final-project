...
export class EventAttendeeService {
  ...
  static async createManyEventAttendees(params: CreateManyEventAttendeeParams) {
    ...
    const existingAttendeesIds = new Set(
      existingAttendees.map((donor) => donor.donorId),
    );
    const newAttendeesData = eventAttendees.filter(
      (eventAttendee) => !existingAttendeesIds.has(eventAttendee.donorId),
    );
  ...
  static async getEventAttendeesByFilter(
    filter: GetEventAttendeeByFilterParams,
  ) {
  ...
  static async deleteEventAttendeesByEventId(
    param: DeleteEventAttendeeByEventIdParams,
  ) {
    try {
